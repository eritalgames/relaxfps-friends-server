...
// Token E: Play Integrity and adaptive account/device risk controls.
// Keep mode "monitor" until the app is distributed through Google Play, then
// switch to "enforce" after confirming production verdicts in Admin Studio.
const PLAY_INTEGRITY_MODE = ['off', 'monitor', 'enforce'].includes(String(process.env.RELAXFPS_PLAY_INTEGRITY_MODE || 'monitor').toLowerCase())
  ? String(process.env.RELAXFPS_PLAY_INTEGRITY_MODE || 'monitor').toLowerCase()
  : 'monitor';
const PLAY_INTEGRITY_PROJECT_NUMBER = String(process.env.RELAXFPS_PLAY_INTEGRITY_PROJECT_NUMBER || '').trim();
const PLAY_INTEGRITY_SCOPE = 'https://www.googleapis.com/auth/playintegrity';
const PLAY_INTEGRITY_API_BASE_URL = String(
  process.env.RELAXFPS_PLAY_INTEGRITY_API_BASE_URL || 'https://playintegrity.googleapis.com',
).replace(/\/+$/g, '');
const PLAY_INTEGRITY_CHALLENGE_TTL_MS = Math.max(
  60 * 1000,
  Math.min(Number(process.env.RELAXFPS_PLAY_INTEGRITY_CHALLENGE_TTL_MS || 5 * 60 * 1000), 15 * 60 * 1000),
);
const PLAY_INTEGRITY_PROOF_TTL_MS = Math.max(
  30 * 1000,
  Math.min(Number(process.env.RELAXFPS_PLAY_INTEGRITY_PROOF_TTL_MS || 3 * 60 * 1000), 10 * 60 * 1000),
);
const PLAY_INTEGRITY_REVIEW_THRESHOLD = Math.max(10, Math.min(Number(process.env.RELAXFPS_RISK_REVIEW_THRESHOLD || 40), 200));
const PLAY_INTEGRITY_LOCK_THRESHOLD = Math.max(20, Math.min(Number(process.env.RELAXFPS_RISK_LOCK_THRESHOLD || 75), 250));
const PLAY_INTEGRITY_BAN_THRESHOLD = Math.max(50, Math.min(Number(process.env.RELAXFPS_RISK_BAN_THRESHOLD || 180), 500));
const PLAY_INTEGRITY_LOCK_MINUTES = Math.max(5, Math.min(Number(process.env.RELAXFPS_RISK_LOCK_MINUTES || 60), 10080));
const PLAY_INTEGRITY_AUTO_BAN = String(process.env.RELAXFPS_RISK_AUTO_BAN || 'false').toLowerCase() === 'true';
const PLAY_INTEGRITY_MAX_RECORDS = 30000;
const PLAY_INTEGRITY_REQUIRED_ACTIONS = Object.freeze(new Set([
  'wallet_enroll',
  'wallet_recover',
  'wallet_spend',
  'wallet_ad_session',
...
let playIntegrityAccessTokenCache = { token: '', expiresAt: 0 };
...
const state = {
  ...
  integrityChallenges: {}, // challengeId -> request binding and expiry
  integrityProofs: {}, // proofId -> verified verdict binding
  integrityTokenIndex: {}, // token HMAC -> proofId, extra replay protection
  securityReviews: [], // admin review queue generated by risk engine
  securityProfiles: {}, // RelaxFPS ID -> accumulated risk state
...
function applyLoadedState(parsed) {
  ...
  state.integrityChallenges = state.integrityChallenges && typeof state.integrityChallenges === 'object' ? state.integrityChallenges : {};
  state.integrityProofs = state.integrityProofs && typeof state.integrityProofs === 'object' ? state.integrityProofs : {};
  state.integrityTokenIndex = state.integrityTokenIndex && typeof state.integrityTokenIndex === 'object' ? state.integrityTokenIndex : {};
  state.securityReviews = Array.isArray(state.securityReviews) ? state.securityReviews : [];
  state.securityProfiles = state.securityProfiles && typeof state.securityProfiles === 'object' ? state.securityProfiles : {};
  ...
  if (req.method === 'GET' && (pathname === '/health' || pathname === '/healthz')) {
    sendJsonResponse(res, 200, {
      ...
      wallet: {
        ...
        playIntegrity: {
          ...playIntegrityConfigSnapshot(),
          challenges: Object.keys(state.integrityChallenges || {}).length,
          proofs: Object.keys(state.integrityProofs || {}).length,
          pendingReviews: (state.securityReviews || []).filter((item) => item.status === 'pending').length,
        },
      },
      persistence: {
...
function walletPublicSnapshot(id) {
  ...
  return {
    ...
    integrity: walletIntegrityStatus.ok,
    riskScore: Math.max(0, Math.round(Number(state.securityProfiles?.[clean]?.score ?? wallet?.riskScore ?? 0))),
...
function playIntegrityConfigured() {
  if (!/^\d{5,24}$/.test(PLAY_INTEGRITY_PROJECT_NUMBER)) return false;
  if (GOOGLE_PLAY_LOCAL_TESTS_ALLOWED && /^http:\/\/(127\.0\.0\.1|localhost)(:\d+)?$/i.test(PLAY_INTEGRITY_API_BASE_URL)) return true;
  return !!googlePlayServiceAccount();
}

function playIntegrityConfigSnapshot() {
  return {
    mode: PLAY_INTEGRITY_MODE,
    configured: playIntegrityConfigured(),
    projectNumber: PLAY_INTEGRITY_PROJECT_NUMBER,
    packageName: GOOGLE_PLAY_PACKAGE_NAME,
    requiredActions: Array.from(PLAY_INTEGRITY_REQUIRED_ACTIONS),
    reviewThreshold: PLAY_INTEGRITY_REVIEW_THRESHOLD,
    lockThreshold: PLAY_INTEGRITY_LOCK_THRESHOLD,
  };
}

function playIntegrityBase64Url(value) {
  return Buffer.from(value).toString('base64url');
}

async function playIntegrityAccessToken() {
  if (GOOGLE_PLAY_LOCAL_TESTS_ALLOWED && /^http:\/\/(127\.0\.0\.1|localhost)(:\d+)?$/i.test(PLAY_INTEGRITY_API_BASE_URL)) {
    return 'local-integrity-test-token';
  }
  const now = Date.now();
  if (playIntegrityAccessTokenCache.token && playIntegrityAccessTokenCache.expiresAt - 60000 > now) {
    return playIntegrityAccessTokenCache.token;
  }
  const account = googlePlayServiceAccount();
  if (!account) throw Object.assign(new Error('Play Integrity service account yapılandırılmadı.'), { code: 'integrity_not_configured' });
  const issuedAt = Math.floor(now / 1000);
  const header = playIntegrityBase64Url(JSON.stringify({ alg: 'RS256', typ: 'JWT' }));
  const payload = playIntegrityBase64Url(JSON.stringify({
    iss: account.clientEmail,
  ...
  const body = await response.json().catch(() => ({}));
  if (!response.ok || !body.access_token) {
    throw Object.assign(new Error(`Play Integrity OAuth başarısız: ${body.error_description || body.error || response.status}`), {
      code: 'integrity_oauth_failed',
      statusCode: response.status,
    });
  }
...
function playIntegrityTokenHash(token) {
  return crypto.createHmac('sha256', WALLET_LEDGER_SECRET)
    .update(`RELAXFPS:PLAY:INTEGRITY:${String(token || '')}`, 'utf8')
    .digest('hex');
}

function playIntegrityPayloadDigest(value) {
  return crypto.createHash('sha256')
    .update(stableJson(normalizeWalletMetadata(value)), 'utf8')
    .digest('base64url');
}

function playIntegrityExpectedHash(challenge) {
  return crypto.createHash('sha256')
    .update(stableJson({
...
function playIntegrityCleanup() {
  const now = Date.now();
  state.integrityChallenges = state.integrityChallenges || {};
  state.integrityProofs = state.integrityProofs || {};
  state.integrityTokenIndex = state.integrityTokenIndex || {};
  for (const [id, item] of Object.entries(state.integrityChallenges)) {
    if (!item || Number(item.expiresAtMs || 0) < now - 60 * 1000) delete state.integrityChallenges[id];
  }
  for (const [id, item] of Object.entries(state.integrityProofs)) {
    if (!item || Number(item.expiresAtMs || 0) < now - 60 * 1000) delete state.integrityProofs[id];
  }
  const challengeEntries = Object.entries(state.integrityChallenges);
  if (challengeEntries.length > PLAY_INTEGRITY_MAX_RECORDS) {
    challengeEntries.sort((a, b) => Number(a[1]?.createdAtMs || 0) - Number(b[1]?.createdAtMs || 0));
    for (const [id] of challengeEntries.slice(0, challengeEntries.length - PLAY_INTEGRITY_MAX_RECORDS)) delete state.integrityChallenges[id];
  }
  const proofEntries = Object.entries(state.integrityProofs);
  if (proofEntries.length > PLAY_INTEGRITY_MAX_RECORDS) {
    proofEntries.sort((a, b) => Number(a[1]?.createdAtMs || 0) - Number(b[1]?.createdAtMs || 0));
    for (const [id, item] of proofEntries.slice(0, proofEntries.length - PLAY_INTEGRITY_MAX_RECORDS)) {
      if (item?.tokenHash) delete state.integrityTokenIndex[item.tokenHash];
      delete state.integrityProofs[id];
    }
  }
}

function playIntegrityCreateChallenge({ id, action, operationId, payload = {}, deviceId = '' }) {
  playIntegrityCleanup();
  const clean = normalizeId(id);
  const cleanAction = walletActionKey(action);
  const cleanOperationId = String(operationId || '').trim().slice(0, 160);
  if (!validId(clean) || !PLAY_INTEGRITY_REQUIRED_ACTIONS.has(cleanAction) || !validWalletRequestId(cleanOperationId)) {
    throw Object.assign(new Error('Geçersiz Integrity işlem bağlamı.'), { code: 'invalid_integrity_context' });
  }
  const createdAtMs = Date.now();
  const challenge = {
    id: `pic-${createdAtMs}-${crypto.randomBytes(10).toString('hex')}`,
    userId: clean,
    action: cleanAction,
    operationId: cleanOperationId,
    payloadDigest: playIntegrityPayloadDigest(payload),
    deviceHash: walletDeviceHash(deviceId),
    issuedAt: new Date(createdAtMs).toISOString(),
    createdAtMs,
    expiresAtMs: createdAtMs + PLAY_INTEGRITY_CHALLENGE_TTL_MS,
    status: 'pending',
  };
  challenge.requestHash = playIntegrityExpectedHash(challenge);
  state.integrityChallenges[challenge.id] = challenge;
  saveStateSoon();
  return {
    id: challenge.id,
...
async function playIntegrityDecodeToken(integrityToken) {
  if (!playIntegrityConfigured()) {
    throw Object.assign(new Error('Play Integrity henüz yapılandırılmadı.'), { code: 'integrity_not_configured' });
  }
  const accessToken = await playIntegrityAccessToken();
  const response = await fetch(
    `${PLAY_INTEGRITY_API_BASE_URL}/v1/${encodeURIComponent(GOOGLE_PLAY_PACKAGE_NAME)}:decodeIntegrityToken`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  if (!response.ok || !body.tokenPayloadExternal) {
    throw Object.assign(new Error(body?.error?.message || body?.message || `Play Integrity HTTP ${response.status}`), {
      code: response.status === 429 ? 'integrity_rate_limited' : 'integrity_decode_failed',
      statusCode: response.status,
    });
  }
...
function playIntegrityVerdictSummary(payload) {
  ...
  const deviceVerdicts = Array.isArray(deviceIntegrity.deviceRecognitionVerdict)
    ...
    : [];
  const appsDetected = Array.isArray(environmentDetails?.appAccessRiskVerdict?.appsDetected)
    ? environmentDetails.appAccessRiskVerdict.appsDetected.map(String)
    : [];
  return {
    requestHash: String(requestDetails.requestHash || ''),
    timestampMillis: Number(requestDetails.timestampMillis || 0),
    appRecognitionVerdict: String(appIntegrity.appRecognitionVerdict || 'UNEVALUATED'),
    packageName: String(appIntegrity.packageName || ''),
    versionCode: String(appIntegrity.versionCode || ''),
    certificateDigest: Array.isArray(appIntegrity.certificateSha256Digest)
      ? appIntegrity.certificateSha256Digest.map(String).slice(0, 4)
      : [],
    appLicensingVerdict: String(accountDetails.appLicensingVerdict || 'UNEVALUATED'),
    deviceVerdicts,
    appsDetected,
    playProtectVerdict: String(environmentDetails.playProtectVerdict || 'UNEVALUATED'),
  };
}

function playIntegrityAssess(summary, challenge) {
  let score = 0;
  const reasons = [];
  const critical = [];
  if (!secureStringEqual(summary.requestHash, challenge.requestHash)) {
    score += 120; reasons.push('request_hash_mismatch'); critical.push('request_hash_mismatch');
  }
  if (summary.packageName && summary.packageName !== GOOGLE_PLAY_PACKAGE_NAME) {
    score += 120; reasons.push('package_mismatch'); critical.push('package_mismatch');
  }
  if (summary.appRecognitionVerdict !== 'PLAY_RECOGNIZED') {
    score += summary.appRecognitionVerdict === 'UNEVALUATED' ? 25 : 70;
    reasons.push(`app_${summary.appRecognitionVerdict.toLowerCase()}`);
  }
  if (summary.appLicensingVerdict !== 'LICENSED') {
    score += summary.appLicensingVerdict === 'UNEVALUATED' ? 15 : 35;
    reasons.push(`license_${summary.appLicensingVerdict.toLowerCase()}`);
  }
  if (!summary.deviceVerdicts.includes('MEETS_DEVICE_INTEGRITY')) {
    score += summary.deviceVerdicts.includes('MEETS_BASIC_INTEGRITY') ? 35 : 65;
    reasons.push('device_integrity_missing');
  }
  if (summary.appsDetected.some((item) => /CAPTURING|CONTROLLING|OVERLAYS/i.test(item))) {
    score += 25; reasons.push('app_access_risk');
  }
  if (summary.playProtectVerdict && !['UNEVALUATED', 'NO_ISSUES'].includes(summary.playProtectVerdict)) {
    score += 30; reasons.push(`play_protect_${summary.playProtectVerdict.toLowerCase()}`);
  }
  const ageMs = summary.timestampMillis > 0 ? Math.abs(Date.now() - summary.timestampMillis) : Number.POSITIVE_INFINITY;
  if (ageMs > 10 * 60 * 1000) {
    score += 40; reasons.push('stale_verdict');
  }
  return { score, reasons, critical, accepted: critical.length === 0 && score < PLAY_INTEGRITY_LOCK_THRESHOLD };
}

function securityProfile(id) {
  const clean = normalizeId(id);
  state.securityProfiles = state.securityProfiles || {};
  const current = state.securityProfiles[clean] && typeof state.securityProfiles[clean] === 'object'
    ? state.securityProfiles[clean]
    : { id: clean, score: 0, incidents: 0, lastUpdatedAt: '', lastVerdictAt: '', status: 'clear' };
  const previousMs = Date.parse(current.lastUpdatedAt || '');
  if (Number.isFinite(previousMs) && previousMs > 0) {
    const decay = Math.floor((Date.now() - previousMs) / (6 * 60 * 60 * 1000));
    if (decay > 0) current.score = Math.max(0, Number(current.score || 0) - decay);
  }
  state.securityProfiles[clean] = current;
  return current;
}

function securityQueueReview(id, score, reasons, summary = {}) {
  state.securityReviews = Array.isArray(state.securityReviews) ? state.securityReviews : [];
  const clean = normalizeId(id);
  const existing = state.securityReviews.find((item) => item.userId === clean && item.status === 'pending');
  if (existing) {
    existing.score = Math.max(Number(existing.score || 0), Number(score || 0));
    existing.reasons = Array.from(new Set([...(existing.reasons || []), ...reasons])).slice(0, 30);
    existing.updatedAt = new Date().toISOString();
    existing.summary = { ...(existing.summary || {}), ...summary };
    return existing;
  }
  const item = {
    id: `review-${Date.now()}-${crypto.randomBytes(5).toString('hex')}`,
    userId: clean,
    score: Number(score || 0),
    reasons: Array.from(new Set(reasons)).slice(0, 30),
    summary,
    status: 'pending',
  ...
  state.securityReviews.push(item);
  if (state.securityReviews.length > 5000) state.securityReviews = state.securityReviews.slice(-5000);
  return item;
}

function securityApplyRisk(id, assessment, summary, context = {}) {
  const clean = normalizeId(id);
  const profile = securityProfile(clean);
  const points = Math.max(0, Math.round(Number(assessment.score || 0)));
  profile.score = Math.min(500, Math.max(0, Math.round(Number(profile.score || 0))) + points);
  profile.incidents = Math.max(0, Math.round(Number(profile.incidents || 0))) + (points > 0 ? 1 : 0);
  profile.lastUpdatedAt = new Date().toISOString();
  profile.lastVerdictAt = profile.lastUpdatedAt;
  profile.lastReasons = assessment.reasons.slice(0, 20);
  profile.lastSummary = summary;
  const enforcementActive = PLAY_INTEGRITY_MODE === 'enforce';
  profile.status = enforcementActive && profile.score >= PLAY_INTEGRITY_LOCK_THRESHOLD ? 'locked'
    : profile.score >= PLAY_INTEGRITY_REVIEW_THRESHOLD ? 'review' : 'clear';
  const wallet = walletRecord(clean);
  if (wallet) wallet.riskScore = profile.score;

  if (points > 0) {
    walletSecurityEvent('play_integrity_risk', {
      id: clean,
      points,
    ...
    }, profile.score >= PLAY_INTEGRITY_LOCK_THRESHOLD ? 'high' : 'warning');
  }
  if (profile.score >= PLAY_INTEGRITY_REVIEW_THRESHOLD) {
    securityQueueReview(clean, profile.score, assessment.reasons, {
      action: context.action || '',
      operationId: context.operationId || '',
      appRecognitionVerdict: summary.appRecognitionVerdict,
      appLicensingVerdict: summary.appLicensingVerdict,
      deviceVerdicts: summary.deviceVerdicts,
      playProtectVerdict: summary.playProtectVerdict,
    });
  }
  if (enforcementActive && wallet && profile.score >= PLAY_INTEGRITY_LOCK_THRESHOLD) {
    wallet.locked = true;
    wallet.lockedUntil = new Date(Date.now() + PLAY_INTEGRITY_LOCK_MINUTES * 60 * 1000).toISOString();
    wallet.lockReason = 'Play Integrity güvenlik incelemesi';
    sendTo(clean, { type: 'wallet_changed', wallet: walletPublicSnapshot(clean), reason: 'security_lock', serverTime: new Date().toISOString() });
  }
  if (enforcementActive && PLAY_INTEGRITY_AUTO_BAN && profile.score >= PLAY_INTEGRITY_BAN_THRESHOLD) {
    state.bannedUsers[clean] = {
      id: clean,
      reason: 'Play Integrity: tekrarlanan kritik güvenlik ihlali',
      until: null,
      time: new Date().toISOString(),
      source: 'token_e_auto_ban',
    };
    profile.status = 'banned';
    sendTo(clean, { type: 'banned', ban: state.bannedUsers[clean] });
  }
  saveStateSoon();
  return profile;
}

async function playIntegrityVerifyChallenge({ id, challengeId, integrityToken, ip = '' }) {
  playIntegrityCleanup();
  const clean = normalizeId(id);
  const challenge = state.integrityChallenges?.[String(challengeId || '')];
  if (!challenge || challenge.userId !== clean) {
    throw Object.assign(new Error('Integrity doğrulama isteği bulunamadı veya süresi doldu.'), { code: 'integrity_challenge_not_found' });
  }
  if (challenge.status !== 'pending' || Number(challenge.expiresAtMs || 0) < Date.now()) {
    throw Object.assign(new Error('Integrity doğrulama isteği daha önce kullanılmış veya süresi dolmuş.'), { code: 'integrity_challenge_reused' });
  }
  const token = String(integrityToken || '').trim();
  if (token.length < 100 || token.length > 100000) {
    throw Object.assign(new Error('Geçersiz Play Integrity belirteci.'), { code: 'invalid_integrity_token' });
  }
  const tokenHash = playIntegrityTokenHash(token);
  if (state.integrityTokenIndex?.[tokenHash]) {
    const assessment = { score: 80, reasons: ['integrity_token_replay'], critical: ['integrity_token_replay'], accepted: false };
    securityApplyRisk(clean, assessment, {}, { action: challenge.action, operationId: challenge.operationId });
    throw Object.assign(new Error('Bu Play Integrity belirteci daha önce kullanıldı.'), { code: 'integrity_token_replay' });
  }

  let payload;
  try {
    payload = await playIntegrityDecodeToken(token);
  } catch (error) {
    challenge.status = 'failed';
    challenge.failureCode = error.code || 'integrity_decode_failed';
    const points = error.code === 'integrity_not_configured' ? 0 : 35;
    if (points > 0) securityApplyRisk(clean, { score: points, reasons: [error.code || 'integrity_decode_failed'], critical: [], accepted: false }, {}, { action: challenge.action, operationId: challenge.operationId });
    walletSecurityEvent('play_integrity_decode_failed', { id: clean, ip, code: error.code || 'error' }, points > 0 ? 'warning' : 'info');
    saveStateSoon();
    throw error;
  }
  const summary = playIntegrityVerdictSummary(payload);
  const assessment = playIntegrityAssess(summary, challenge);
  const profile = securityApplyRisk(clean, assessment, summary, { action: challenge.action, operationId: challenge.operationId });
  challenge.status = 'verified';
  challenge.verifiedAt = new Date().toISOString();
  challenge.tokenHash = tokenHash;
  const proofId = `pip-${Date.now()}-${crypto.randomBytes(10).toString('hex')}`;
  const proof = {
    id: proofId,
    userId: clean,
    challengeId: challenge.id,
    ...
    riskPoints: assessment.score,
    reasons: assessment.reasons,
    summary,
    createdAtMs: Date.now(),
    expiresAtMs: Date.now() + PLAY_INTEGRITY_PROOF_TTL_MS,
    consumed: false,
  };
  state.integrityProofs[proofId] = proof;
  state.integrityTokenIndex[tokenHash] = proofId;
  await saveStateDurable();
  return {
    proofId,
    accepted: proof.accepted,
    riskPoints: proof.riskPoints,
    riskScore: profile.score,
    reasons: proof.reasons,
    expiresAt: new Date(proof.expiresAtMs).toISOString(),
    summary,
  };
}

function playIntegrityAuthorizeOperation({ id, action, operationId, proofId, payload = {}, ip = '' }) {
  const clean = normalizeId(id);
  const cleanAction = walletActionKey(action);
  if (PLAY_INTEGRITY_MODE === 'off' || !PLAY_INTEGRITY_REQUIRED_ACTIONS.has(cleanAction)) {
    return { ok: true, enforced: false, monitored: false };
  }
  if (!playIntegrityConfigured()) {
    if (PLAY_INTEGRITY_MODE === 'enforce') {
      return { ok: false, code: 'integrity_not_configured', message: 'Play Integrity sunucuda yapılandırılmadı.' };
    }
    return { ok: true, enforced: false, monitored: true, missing: true };
  }
  const proof = state.integrityProofs?.[String(proofId || '')];
  if (!proof) {
    walletSecurityEvent('play_integrity_proof_missing', { id: clean, action: cleanAction, ip }, PLAY_INTEGRITY_MODE === 'enforce' ? 'high' : 'info');
    return PLAY_INTEGRITY_MODE === 'enforce'
      ? { ok: false, code: 'integrity_proof_required', message: 'Bu işlem için Play Integrity doğrulaması gerekli.' }
      : { ok: true, enforced: false, monitored: true, missing: true };
  }
  const expectedDigest = playIntegrityPayloadDigest(payload);
  const mismatch = proof.userId !== clean
    || proof.action !== cleanAction
    || proof.operationId !== String(operationId || '')
    || proof.payloadDigest !== expectedDigest;
  if (mismatch) {
    securityApplyRisk(clean, { score: 100, reasons: ['integrity_proof_binding_mismatch'], critical: ['integrity_proof_binding_mismatch'], accepted: false }, proof.summary || {}, { action: cleanAction, operationId });
    return { ok: false, code: 'integrity_proof_mismatch', message: 'Integrity doğrulaması bu işleme ait değil.' };
  }
  if (proof.consumed || Number(proof.expiresAtMs || 0) < Date.now()) {
    securityApplyRisk(clean, { score: 60, reasons: ['integrity_proof_replay'], critical: ['integrity_proof_replay'], accepted: false }, proof.summary || {}, { action: cleanAction, operationId });
    return { ok: false, code: 'integrity_proof_reused', message: 'Integrity doğrulaması daha önce kullanıldı veya süresi doldu.' };
  }
  if (!proof.accepted && PLAY_INTEGRITY_MODE === 'enforce') {
    return { ok: false, code: 'integrity_verdict_rejected', message: 'Cihaz veya uygulama güvenlik doğrulamasından geçemedi.', riskScore: securityProfile(clean).score };
  }
  proof.consumed = true;
  proof.consumedAt = new Date().toISOString();
  saveStateSoon();
  return { ok: true, enforced: PLAY_INTEGRITY_MODE === 'enforce', monitored: PLAY_INTEGRITY_MODE === 'monitor', proof };
}
...
function adminSnapshot() {
  ...
  return {
    ...
    securityReviews: (state.securityReviews || []).slice().sort((a, b) => String(b.updatedAt || b.createdAt || '').localeCompare(String(a.updatedAt || a.createdAt || ''))).slice(0, 500),
    securityProfiles: Object.values(state.securityProfiles || {}).sort((a, b) => Number(b.score || 0) - Number(a.score || 0)).slice(0, 1000),
    playIntegrity: playIntegrityConfigSnapshot(),
    ...
    systemHealth: {
      ...
      playIntegrityMode: PLAY_INTEGRITY_MODE,
      playIntegrityConfigured: playIntegrityConfigured(),
      pendingSecurityReviews: (state.securityReviews || []).filter((item) => item.status === 'pending').length,
  ...
  socket.on('message', async (raw) => {
    ...
    if (type === 'wallet_integrity_challenge') {
      ...
      if (!validId(id) || currentId !== id) {
        send(socket, { type: 'wallet_integrity_challenge', ok: false, requestId, code: 'identity_required', message: 'Önce aynı RelaxFPS ID ile sunucuya bağlan.' });
        return;
      }
      if (!PLAY_INTEGRITY_REQUIRED_ACTIONS.has(action) || !validWalletRequestId(operationId)) {
        send(socket, { type: 'wallet_integrity_challenge', ok: false, requestId, code: 'invalid_integrity_context', message: 'Geçersiz güvenlik işlem bağlamı.' });
        return;
      }
      if (!['wallet_enroll', 'wallet_recover'].includes(action)) {
        const wallet = walletAuthenticate(socket, socketIp, id, data.walletKey, requestId, 'wallet_integrity_challenge');
        if (!wallet) return;
      }
      const rate = walletRateLimit(`integrity-challenge:${socketIp}:${id}`, 20, 10 * 60 * 1000, 15 * 60 * 1000);
      if (!rate.ok) {
        send(socket, { type: 'wallet_integrity_challenge', ok: false, requestId, code: 'rate_limited', retryAfterSeconds: rate.retryAfterSeconds, message: 'Çok fazla güvenlik doğrulama isteği.' });
        return;
      }
      try {
        const challenge = playIntegrityCreateChallenge({
          id,
          action,
          operationId,
        ...
        send(socket, {
          type: 'wallet_integrity_challenge',
          ok: true,
          requestId,
          challenge,
          playIntegrity: playIntegrityConfigSnapshot(),
        ...
        send(socket, { type: 'wallet_integrity_challenge', ok: false, requestId, code: error.code || 'integrity_challenge_failed', message: error.message });
      }
      return;
    }

    if (type === 'wallet_integrity_verify') {
      const id = normalizeId(currentId || data.id);
      if (!validId(id) || currentId !== id) {
        send(socket, { type: 'wallet_integrity_verify', ok: false, requestId, code: 'identity_required', message: 'Önce aynı RelaxFPS ID ile sunucuya bağlan.' });
        return;
      }
      const rate = walletRateLimit(`integrity-verify:${socketIp}:${id}`, 20, 10 * 60 * 1000, 30 * 60 * 1000);
      if (!rate.ok) {
        send(socket, { type: 'wallet_integrity_verify', ok: false, requestId, code: 'rate_limited', retryAfterSeconds: rate.retryAfterSeconds, message: 'Çok fazla Play Integrity doğrulaması.' });
        return;
      }
      try {
        const result = await playIntegrityVerifyChallenge({
          id,
        ...
        send(socket, {
          ...
          ok: false,
          requestId,
          code: error.code || 'integrity_verify_failed',
    ...
    if (type === 'wallet_enroll') {
      ...
      const integrityAuth = playIntegrityAuthorizeOperation({
        id, action: 'wallet_enroll', operationId, proofId: data.integrityProofId,
        payload: { deviceId: String(data.deviceId || '') }, ip: socketIp,
      });
      if (!integrityAuth.ok) {
        send(socket, { type: 'wallet_enroll', ok: false, requestId, code: integrityAuth.code, message: integrityAuth.message, wallet: walletPublicSnapshot(id) });
        return;
      }
      try {
    ...
    if (type === 'wallet_recover') {
      ...
      const operationId = String(data.operationId || requestId || '').trim();
      const integrityAuth = playIntegrityAuthorizeOperation({
        id, action: 'wallet_recover', operationId, proofId: data.integrityProofId,
        payload: { deviceId: String(data.deviceId || '') }, ip: socketIp,
      });
      if (!integrityAuth.ok) {
        send(socket, { type: 'wallet_recover', ok: false, requestId, code: integrityAuth.code, message: integrityAuth.message, wallet: walletPublicSnapshot(id) });
        return;
      }
      try {
    ...
    if (type === 'wallet_ad_session') {
      ...
      const operationId = String(data.operationId || requestId || '').trim();
      const integrityAuth = playIntegrityAuthorizeOperation({
        id, action: 'wallet_ad_session', operationId, proofId: data.integrityProofId,
        payload: {
      ...
      if (!integrityAuth.ok) {
        send(socket, { type: 'wallet_ad_session', ok: false, requestId, code: integrityAuth.code, message: integrityAuth.message, wallet: walletPublicSnapshot(id), adState: walletAdStateSnapshot(id, data.timezoneOffsetMinutes) });
        return;
      }
      try {
    ...
    if (type === 'wallet_play_verify') {
      ...
      const operationId = String(data.operationId || requestId || '').trim();
      const integrityAuth = playIntegrityAuthorizeOperation({
        id, action: 'wallet_play_verify', operationId, proofId: data.integrityProofId,
        payload: {
      ...
      if (!integrityAuth.ok) {
        send(socket, { type: 'wallet_play_verify', ok: false, requestId, code: integrityAuth.code, message: integrityAuth.message, wallet: walletPublicSnapshot(id) });
        return;
      }
      try {
    ...
    if (type === 'admin_security_review_action') {
      if (!requireAdmin(socket, adminSessionToken, requestId)) return;
      const reviewId = String(data.reviewId || '').trim();
      const action = String(data.action || '').trim().toLowerCase();
      const review = (state.securityReviews || []).find((item) => item.id === reviewId);
      if (!review) {
        send(socket, { type: 'admin_error', ok: false, requestId, code: 'review_not_found', message: 'Güvenlik inceleme kaydı bulunamadı.' });
        return;
      }
      const id = normalizeId(review.userId);
      const profile = securityProfile(id);
      const wallet = walletRecord(id);
      const now = new Date().toISOString();
      if (action === 'approve' || action === 'reset') {
        ...
        review.resolution = 'approved_reset';
      } else if (action === 'lock') {
        const minutes = Math.max(5, Math.min(Number(data.minutes || PLAY_INTEGRITY_LOCK_MINUTES), 10080));
        if (wallet) {
          wallet.locked = true;
          wallet.lockedUntil = new Date(Date.now() + minutes * 60 * 1000).toISOString();
          wallet.lockReason = String(data.reason || 'Yönetici güvenlik incelemesi').slice(0, 300);
        }
        profile.status = 'locked';
        review.status = 'resolved';
        review.resolution = `locked_${minutes}m`;
      } else if (action === 'ban') {
        state.bannedUsers[id] = {
          id,
          reason: String(data.reason || 'Play Integrity güvenlik ihlali').slice(0, 400),
          until: null,
          time: now,
          source: 'token_e_admin_review',
        };
        profile.status = 'banned';
        review.status = 'resolved';
        review.resolution = 'permanent_ban';
        sendTo(id, { type: 'banned', ban: state.bannedUsers[id] });
      } else if (action === 'dismiss') {
        review.status = 'resolved';
        review.resolution = 'dismissed';
      } else {
        send(socket, { type: 'admin_error', ok: false, requestId, code: 'invalid_review_action', message: 'Geçersiz inceleme işlemi.' });
        return;
      }
      review.resolvedAt = now;
      review.updatedAt = now;
      review.adminNote = String(data.note || '').slice(0, 1000);
      if (!await saveStateDurable()) {
        send(socket, { type: 'admin_error', ok: false, requestId, code: 'security_review_persistence_failed', message: 'Güvenlik kararı kalıcı veritabanına yazılamadı.' });
        return;
      }
      walletSecurityEvent('admin_security_review_resolved', { id, reviewId, action, resolution: review.resolution }, 'info');
      adminAudit('security_review_action', { id, reviewId, action, resolution: review.resolution });
      if (wallet) sendTo(id, { type: 'wallet_changed', wallet: walletPublicSnapshot(id), reason: 'security_review', serverTime: now });
      send(socket, { type: 'admin_security_review_action', ok: true, requestId, review, wallet: walletPublicSnapshot(id), profile });
      return;
    }
...
async function bootstrapServer() {
  ...
  httpServer.listen(PORT, () => {
    ...
    console.log(`[PLAY INTEGRITY] mode=${PLAY_INTEGRITY_MODE}, configured=${playIntegrityConfigured()}, package=${GOOGLE_PLAY_PACKAGE_NAME}.`);
    if (googlePlayConfigured()) {
